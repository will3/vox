...
namespace FarmVox
{
    public class CubeMeshBuilder
    {
        ...
        public CubeMeshBuilder AddQuad(Axis d, bool front, Vector3 offset) {
            ...
            var v0 = GetVector(diffI, 0, 0, d) + offset;
            var v1 = GetVector(diffI, 1.0f, 0, d) + offset;
            var v2 = GetVector(diffI, 1.0f, 1.0f, d) + offset;
            var v3 = GetVector(diffI, 0, 1.0f, d) + offset;
        ...
        private static Vector3 GetDir(Axis d, bool front, float scale = 0.1f)
        {
            var multiplier = front ? 1.0f : -1.0f;
            switch (d)
            {
                case Axis.X:
                    return new Vector3(1, 0, 0) * scale * multiplier;
                case Axis.Y:
                    return new Vector3(0, 1, 0) * scale * multiplier;
                case Axis.Z:
                    return new Vector3(0, 0, 1) * scale * multiplier;
                default:
                    throw new ArgumentOutOfRangeException("d", d, null);
            }
        }
